...
class Image < ActiveRecord::Base
  ...
  def to_arff
    "#{id}, #{results.average(:psi)*1000}, #{trip_diff_wrapper(:clielch).join(', ')}, #{trip_diff_wrapper(:clielch, :square).join(', ')}, #{trip_diff_wrapper(:clielch, :abs).join(', ')}" if results.average(:psi)
  end
